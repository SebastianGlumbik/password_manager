...
export default function Main(): JSX.Element {
    ...
    const [compromisedRecords] = createResource(allRecords,async () => await invoke<number[]>("get_compromised_records"), { initialValue: [] });
    const compromisedExists = createMemo(() => !compromisedRecords.loading && compromisedRecords()?.length as number > 0);
                    ...
                    <Show when={filteredRecords()?.length as number != 0} fallback={
                        ...
                        <div class="grid grid-cols-1 w-full rounded-md border border-[#E4E4E4] dark:border-[#4E4E4E] bg-[#E9E9E9] dark:bg-[#454545]">
                            <For each={filteredRecords()}>{(item, index) =>
                                <>
                                <div class={`cursor-pointer ${(selected() as Record)?.id === item.id ? 'bg-[#0064E1] text-[#E9E9E9]' : ''} last:rounded-b-md first:rounded-t-md`}
