    ...
    return (
        <div class="h-full flex flex-col items-center justify-center">
            ...
            <form class="w-80 text-center" onSubmit={async (event) => {
                ...
                setLoading(true);
                try {
                    await invoke("register", { password: password(), confirm_password: confirmPassword() })
                } catch (e) {
                ...
                <div class="relative w-full my-3">
                    <input placeholder="Enter your master password" type={visibility() ? 'text' : 'password'} class="pl-4 w-full h-7 rounded-xl pr-10"
                           onInput={async (event) => {
                ...
                <div class="relative w-full">
                    <input placeholder="Confirm your master password" type={confirmVisibility() ? 'text' : 'password'} class="pl-4 w-full h-7 rounded-xl pr-10"
                           onInput={(event) => {
                ...
                <button type="submit" class="mt-2 flex flex-row items-center justify-center gap-2 h-7 w-32 rounded-xl mx-auto bg-[#3578F7] hover:bg-[#2F6AE1] text-[#E9E9E9]" title="Enter" disabled={loading()}>
                    ...
                    <GetSVG name={loading() ? "spinner" : "right-to-bracket"} class={`w-5 fill-[#E9E9E9] ${loading() ? 'animate-spin' : ''}`} />
                </button>
            </form>
        </div>
    )
