...
function CloudForm() {
    ...
    return (
        <form class="w-80 text-center" onSubmit={async (event) => {
            ...
            try {
                if (status()) {
                    await invoke("disable_cloud");
                    await message("Cloud disabled successfully!", {title: "Success", type: "info"})
                }
                else {
                    await invoke("enable_cloud", { address: address(), username: username(), password: password() });
                }
            ...
            <input placeholder="IP address with port number" type="text" class="pl-4 w-full h-7 rounded-xl pr-10" value={address()}
                   onInput={async (event) => {
            ...
            <div class="relative w-full">
                <input placeholder="Password" type={visibility() ? 'text' : 'password'} class="pl-4 w-full h-7 rounded-xl pr-10" value={password()}
                       onInput={(event) => {
                           setPassword(event.target.value);
                       }}>
                </input>
            ...
            <p class="text-[14px] text-[#EB5545]">{error()}</p>
            <button type="submit" class="mt-2 flex flex-row items-center justify-center gap-2 h-7 w-40 rounded-xl mx-auto bg-[#3578F7] hover:bg-[#2F6AE1] text-[#E9E9E9]" title={!status() ? "Setup" : "Remove"} disabled={loading()}>
                <span class="bg-inherit">{!status() ? "Setup" : "Remove"}</span>
                <GetSVG name={loading() ? "spinner" : (!status() ? "cloud" : "xmark")} class={`w-4 fill-[#E9E9E9] ${loading() ? 'animate-spin' : ''}`} />
            </button>
        </form>
    )
}